...
const updateGroupIdDeviceIdMap = function (
  ...
  audioDeviceList.forEach((deviceObject) => {
    if (deviceObject.kind === "audioinput") {
      availableAudioDevicesDeviceIdGroupIdMap[deviceObject.deviceId] =
        deviceObject.groupId;
  ...
  return new Promise((resolve, reject) => {
    if (navigator.mediaDevices.getUserMedia) {
      navigator.mediaDevices
        ...
        .catch((err) => {
          Plivo.log.error(
            `${LOGCAT.CALL} | Failed to get user media during init ${err.message}`
          );
          reject(err.name);
        });
    } else {
...
export const speechListeners = function (): void {
  ...
  client.speechRecognition.onerror = (error) => {
    Plivo.log.error(
      `${LOGCAT.CALL} | Error in Recognizing speech :`,
      error.error
    );
    if (error.error === "network") {
      client._currentSession?.setSpeechState(
        client._currentSession.SPEECH_STATE.STOPPED_DUE_TO_NETWORK_ERROR
      );
      Plivo.log.info(
        `${LOGCAT.CALL} | Speech Recognition stopped due to network disruption`
      );
    }
  };
  ...
  client.speechRecognition.onstart = () => {
    ...
    if (
      ...
      client._currentSession?.setSpeechState(
        client._currentSession.SPEECH_STATE.RUNNING
      );
    }
  };
  ...
  client._currentSession?.setSpeechState(
    client._currentSession.SPEECH_STATE.STARTING
  );
...
export const replaceStream = function (
  client: Client,
  constraints: any
): Promise<void> {
  return new Promise((resolve, reject) => {
    const pc = client._currentSession
      ? client._currentSession.session.connection
      : null;
    if (!pc) {
      Plivo.log.debug(
        `${LOGCAT.CALL_QUALITY} | session is not active while replacing stream`
      );
      resolve();
    } else if (
      navigator &&
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          Plivo.log.debug(
            `${LOGCAT.CALL_QUALITY} | Got new stream while replacing stream`
          );
          let sender: any = null;
          ...
          if (currentLocalStream) {
            Plivo.log.debug(
              `${LOGCAT.CALL_QUALITY} | Stopping current local stream in replacing stream`
            );
          ...
          if (sender) {
            ...
            client.noiseSuppresion
              .updateProcessingStream(stream)
              .then((updatedStream: MediaStream | null) => {
                ...
                if (currentAudioState === false) {
                  Plivo.log.debug(
                    `${LOGCAT.CALL_QUALITY} | call was muted before replacing stream. Muting new stream`
                  );
                ...
                if (client._currentSession && client._currentSession.stats) {
                  ...
                  client._currentSession.stats.localAudioLevelHelper =
                    new AudioLevel(
                ...
                sender.replaceTrack(stream.getAudioTracks()[0]).catch(() => {
                  Plivo.log.debug(
                    `${LOGCAT.CALL_QUALITY} | error in replacing track`
                  );
        ...
        .catch((err) => {
          Plivo.log.debug(
            `${LOGCAT.CALL_QUALITY} | error in replacing stream ${err}`
          );
          reject(err);
        })
        .then(() => {
        ...
        .catch((err) => {
          Plivo.log.debug(
            `${LOGCAT.CALL_QUALITY} | main error in replacing stream ${err}`
          );
          reject(err);
        });
    } else {
      Plivo.log.debug(
        `${LOGCAT.CALL_QUALITY} | No getUserMedia support in replaceStream`
      );
...
const replaceAudioTrack = function (
  ...
  Plivo.log.debug(
    `${LOGCAT.CALL_QUALITY} |inside replacetrack with device id  : ${deviceId}`
  );
      ...
      (Plivo.audioConstraints as any).optional.forEach((e: any) => {
        Object.entries(e).forEach(([constraint, value]) => {
          if (Plivo.audioConstraints) {
            constraint = constraint.replace(/goog(.)/i, (_, match) =>
              match.toLowerCase()
            );
  ...
  return navigator.mediaDevices.enumerateDevices().then((devices) => {
    ...
    if (deviceInfo.audioOutputIdSet === "") {
      deviceInfo.activeOutputAudioDevice =
        outputDeviceIdLabelMap.get("default");
    } else {
...
export const updateWindowDeviceList = function (deviceList): void {
  ...
  if (defaultInputGroupId !== defaultOutputGroupId && setDevice) {
    ...
    if (groupIdDeviceId[defaultInputGroupId]) {
      clientObject?.audio.speakerDevices.set(
        groupIdDeviceId[defaultInputGroupId]
      );
      setByWindows = true;
      Plivo.log.debug(
        `Updated the windows audio device with id ${groupIdDeviceId[defaultInputGroupId]}`
      );
    }
  }
};
 ...
 */
export const setAudioDeviceForForWindows = function (
  devices,
  lastConnectedMicDevice,
  lastConnectedSpeakerDevice
): void {
  if (
    (lastConnectedMicDevice === "" || lastConnectedMicDevice === "default") &&
    (lastConnectedSpeakerDevice === null ||
      lastConnectedSpeakerDevice === "default" ||
      setByWindows)
  ) {
...
export const checkAudioDevChange = function (): void {
  ...
  const lastActiveSpeakerDevice = clientObject
    ? clientObject.audio.speakerDevices.get()
    : "";
  const lastConnectedMicDevice = clientObject
    ? clientObject.audio.microphoneDevices.get()
    : "";
  audioDevDictionary()
    .then((deviceInfo: DeviceDictionary) => {
      ...
      if (availableAudioDevices && devices) {
          ...
          */
            if (!/default/i.test(device.deviceId)) {
              ...
              if (device.kind === "audioinput") {
                Plivo.log.info(
                  `${LOGCAT.CALL_QUALITY} Audio input device added:- `,
                  JSON.stringify(device)
                );
                setTimeout(() => {
                  if (client && (isFirefox || isSafari)) {
                    Plivo.log.debug(
                      `${LOGCAT.CALL_QUALITY} Setting mic to ${device.deviceId} in firefox/safari`
                    );
                    client.audio.microphoneDevices.set(device.deviceId);
                  } else if (client) {
                    Plivo.log.debug(
                      `${LOGCAT.CALL_QUALITY} Setting mic to ${
                        isWindows && !client.options.useDefaultAudioDevice
                          ? device.deviceId
                          : "default"
                      } `
                    );
                    client.audio.microphoneDevices.set(
                      isWindows && !client.options.useDefaultAudioDevice
                        ? device.deviceId
                        : "default"
                    );
                ...
                Plivo.log.info(
                  `${LOGCAT.CALL_QUALITY} Audio output device added:- `,
                  JSON.stringify(device)
                );
                setTimeout(() => {
                  if (client && isFirefox) {
                    ...
                    Plivo.log.debug(
                      `${LOGCAT.CALL_QUALITY} Setting speaker to ${device.deviceId} in firefox`
                    );
                  } else if (client && !isSafari && !isFirefox) {
                    Plivo.log.debug(
                      `${LOGCAT.CALL_QUALITY} Setting speaker to ${
                        isWindows && !client.options.useDefaultAudioDevice
                          ? device.deviceId
                          : "default"
                      } `
                    );
                    client.audio.speakerDevices.set(
                      isWindows && !client.options.useDefaultAudioDevice
                        ? device.deviceId
                        : "default"
                    );
        ...
        availableAudioDevices.forEach((device) => {
          if (!devices.filter((a) => a.deviceId === device.deviceId).length) {
            // If not present
            ...
            if (!/default/i.test(device.deviceId)) {
              ...
              if (device.kind === "audioinput") {
                Plivo.log.info(
                  `${LOGCAT.CALL_QUALITY} Audio input device removed:- `,
                  JSON.stringify(device)
                );
                Plivo.log.debug(
                  `${LOGCAT.CALL_QUALITY} Setting microphone to default`
                );
                setTimeout(() => {
                  if (client && (isFirefox || isSafari)) {
                    Plivo.log.debug(
                      `${LOGCAT.CALL_QUALITY} Setting microphone to ${availableAudioDevices[0].deviceId} in firefox/safari`
                    );
                    client.audio.microphoneDevices.set(
                      availableAudioDevices[0].deviceId
                    );
                  } else if (client) {
                    Plivo.log.debug(
                      `${LOGCAT.CALL_QUALITY} Setting microphone to default`
              ...
              if (device.kind === "audiooutput") {
                if (
                  lastActiveSpeakerDevice !== "" &&
                  lastActiveSpeakerDevice !== "default" &&
                  clientObject
                ) {
                  if (!client._currentSession) {
                    const initialRemoteView =
                      document.getElementById(REMOTE_VIEW_ID);
                    initialRemoteView?.remove();
                    setupRemoteView();
                    Plivo.log.debug("Remote view id removed");
                  }
                  clientObject.remoteView =
                    document.getElementById(REMOTE_VIEW_ID);
                }
                Plivo.log.info(
                  `${LOGCAT.CALL_QUALITY} Audio output device removed:- `,
                  JSON.stringify(device)
                );
                setTimeout(() => {
                  if (client && isFirefox) {
                    availableAudioDevices.every((deviceObj) => {
                      if (deviceObj.kind === "audiooutput") {
                        Plivo.log.debug(
                          `${LOGCAT.CALL_QUALITY} Setting output to default ${deviceObj.deviceId} in firefox`
                        );
                        if (client)
                          client.audio.speakerDevices.set(deviceObj.deviceId);
                        return false;
                      }
                      return true;
                    });
                    ...
                    Plivo.log.debug(
                      `${LOGCAT.CALL_QUALITY} Setting output to default`
                    );
...
export const inputDevices = ((): InputDevices => ({
  set(deviceId) {
    ...
    const device = availableAudioDevices.filter(
    ...
    if (clientObject) {
      if (deviceId === "default") {
        ...
        replaceAudioTrack(
          ...
          clientObject,
          "added",
          activeDeviceIdDeviceLabelMap[deviceId]
        );
      } else {
        replaceAudioTrack(
          deviceId,
      ...
      getAudioDevicesInfo
        .call(clientObject)
        .then((toggledDeviceInfo: DeviceAudioInfo) => {
          const clientObj = clientObject;
          if (clientObj !== null) {
            Plivo.log.info(
              `${LOGCAT.CALL} | Audio device toggled to`,
              JSON.stringify(device)
            );
            const obj = {
              msg: "AUDIO_DEVICES_TOGGLE",
              deviceInfo: toggledDeviceInfo,
            };
  ...
  reset() {
    if ((Plivo.audioConstraints as any).optional) {
      (Plivo.audioConstraints as any).optional = (
        Plivo.audioConstraints as any
      ).optional.filter((e: { sourceId: any }) => !e.sourceId);
 ...
 */
export const audioDevDicSetter = function (setter: (set: any) => void): void {
...
export const updateGain = function (gainValue: number): void {
  if (currentLocalStream) {
    ...
    // Create a new audio context to manipulate the gain
    ...
    // Create a new MediaStream from the destination's stream
    const updatedStream = destination.stream;
    // Stop the existing tracks
    currentLocalStream.getTracks().forEach((track) => track.stop());
    // Replace currentLocalStream with the new stream with updated gain
    currentLocalStream = updatedStream;
  }
};
